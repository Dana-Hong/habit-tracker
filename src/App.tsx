...
function App() {
  const [habitList, setHabitList] = useState<[] | Habit[]>(
    JSON.parse(localStorage.getItem("habitList") as string)
  );
  const [habitName, setHabitName] = useState<string>("");
  const [previousDate] = useState<Date>(
    new Date(JSON.parse(localStorage.getItem("prevDate") as string))
  );
          ...
          <div className="flex flex-col gap-2">
            {habitList.map((habitItem, habitIndex) => (
              <div key={habitIndex}>
                    ...
                    setHabitList((prevHabitList) => {
                      return prevHabitList.map((habitItem, index) => {
                        return habitIndex === index
                          ? { ...habitItem, showMenu: !habitItem.showMenu }
                          : habitItem;
                      });
                    });
                  }}
                ...
                <div
                  className={`${
                    habitItem.showMenu ? "" : "hidden"
                  } p-2 border border-t-black bg-gray-300`}
                >
